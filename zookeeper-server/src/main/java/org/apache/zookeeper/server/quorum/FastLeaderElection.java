...
public class FastLeaderElection implements Election {
     ...
     * current vote.
     * 投票pk,pk规则如下：
     * 1. 首先比较Epoch,哪个epoch大胜出，epoch代表了当Leader的伦次，是一个递增的，epoch越大意味着数据越新，leader数据越新则可以减少
     * 后续数据同步的效率，当然优先先为leader
     * 2. 然后是比较zxid, 由于zxid = epoch+counter， 第一步已经把epoch比较过了，其实这个步骤只是相当于比较counter大小，counter越大代表数据越新，
     * 优先选为leader
     * 3. 如果前两个指标都没法比较出来，只能通过sid来确定，zxid相等说明两个服务器的数据是一致的，所以选哪个当leader是没有区别的， 这里就选择sid大的当leader
     */
